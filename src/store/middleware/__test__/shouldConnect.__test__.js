...
describe('Middleware - unit - shouldConnect', () => {
  ...
  it('returns correct result for valid config variations', function (done) {
    ...
    assert.equal(
      shouldConnect({}, 'getState', levels.CORE),
      true
    )
  ...
  it('excludes correctly method by "excludeLevel"', function (done) {
    ...
    assert.equal(
      shouldConnect({excludeLevel: [levels.CORE]}, 'otherMethod', levels.STORE),
      true
    )
  ...
  it('includes correctly method when "onlyLevel" is used', function (done) {
    ...
    assert.equal(
      shouldConnect({onlyLevel: [levels.ACTION_HANDLERS]}, 'someAction', levels.ACTION_HANDLERS),
      true
    )

    assert.equal(
      shouldConnect({onlyLevel: [levels.ACTION_HANDLERS]}, 'otherMethod', levels.STORE),
      false
    )
